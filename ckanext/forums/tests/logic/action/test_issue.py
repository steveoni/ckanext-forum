...
from ckanext.forums.logic.action.action import _get_recipients
from ckanext.forums.tests.fixtures import issues_setup, user
...
class TestIssueNewWithEmailing(object):
    ...
    def config(self, ckan_config):
        ckan_config['ckanext.forums.send_email_notifications'] = 'True'
        return ckan_config
    ...
    def test_issue_create(self):
        ...
        with mock.patch('ckanext.forums.logic.action.action.render_jinja2') \
                as render_mock:
            issue_create_result = toolkit.get_action('forum_create')(
    ...
    def test_issue_create_second(self, user, dataset):
        issue_0 = toolkit.get_action('forum_create')(
        ...
        issue_1 = toolkit.get_action('forum_create')(
            context={'user': user['name']},
            data_dict={
                'title': 'Title',
                'description': 'Description',
    ...
    def test_issue_create_multiple_datasets(self, user, dataset):
        ...
        issue_2 = toolkit.get_action('forum_create')(
            context={'user': user['name']},
            data_dict={
                'title': 'Title',
                'description': 'Description',
    ...
    def test_issue_create_dataset_does_not_exist(self, user):
        forum_create = toolkit.get_action('forum_create')
        pytest.raises(
...
class TestIssueComment(object):
    ...
    def _apply_config_changes(cls, cfg):
        ...
        cfg['ckanext.forums.send_email_notifications'] = True
...
class TestIssueSearch(object):
    ...
    def test_list_all_issues_for_dataset(self, user, dataset):
        created_issues = [issue_factories.Issue(user=user, user_id=user['id'],
                                                ...
                                                description=i)
                          for i in range(0, 10)]
        search_res = helpers.call_action('forum_search',
                                         context={'user': user['name']},
                                         dataset_id=dataset['id'],
                                         sort='oldest')
        forum_list = search_res['results']
        assert [i['id'] for i in created_issues] == \
             [i['id'] for i in forum_list]
    ...
    def test_list_all_issues_for_organization(self, user):
        ...
        created_issues = [issue_factories.Issue(user=user, user_id=user['id'],
                                                ...
                                                description=i)
                          for i in range(0, 10)]
        forum_list = helpers.call_action('forum_search',
                                          context={'user': user['name']},
                                          organization_id=org['id'],
                                          sort='oldest')['results']
        assert [i['id'] for i in created_issues] == [i['id'] for i in forum_list]
    ...
    def test_filter_least_commented(self, user, dataset):
        ...
        assert reordered_ids == [i['id'] for i in forum_list]
        assert [1, 2, 3] == [i['comment_count'] for i in forum_list]
    ...
    def test_filter_most_commented(self, user, dataset):
        ...
        assert reordered_ids == [i['id'] for i in forum_list]
        assert [3, 2, 1, 0] == [i['comment_count'] for i in forum_list]
    ...
    def test_filter_by_title_string_search(self, user, dataset):
        ...
        filtered_issues = helpers.call_action('forum_search',
                                              context={'user': user['name']},
                                              dataset_id=dataset['id'],
                                              q='title')['results']
...
class TestIssueDelete(object):
    ...
    def test_deletion(self, user, dataset):
        ...
        helpers.call_action('forum_delete',
                            context={'user': user['name']},
                            dataset_id=dataset['id'],
                            issue_number=issue['number'])
...
class TestCommentSearch(object):
    ...
    def test_reported_search_for_org(self, organization, comment1):
        result = helpers.call_action('forum_comment_search',
                                     organization_id=organization['id'],
                                     only_hidden=True)
