...
class TestAssign(object):
    ...
    def test_user_self_assign(self, app):
        owner = factories.User()
        org = factories.Organization(user=owner)
        dataset = factories.Dataset(user=owner, owner_org=org['id'])
        ...
        print('-============')
        form['assignee'] = owner['name']
        response = helpers.submit_and_follow(app, form, env)
    ...
    #     assert 401 == response.status_int
    ...
    #     response = helpers.submit_and_follow(self.app, form, env)
    #     assert 'User not a user does not exist' in response
