...
from ckanext.forums.model import IssueFilter
from ckanext.forums import model as issuemodel
...
def forum_enabled(dataset):
    ...
    datasets_with_issues_enabled = set(toolkit.aslist(
        config.get('ckanext.forums.enabled_for_datasets')
    ))
    organizations_with_issues_enabled = set(toolkit.aslist(
        config.get('ckanext.forums.enabled_for_organizations')
    ))
    if datasets_with_issues_enabled or organizations_with_issues_enabled:
        ...
        for extra in extras:
            if extra.get('key') == 'forum_enabled':
                return toolkit.asbool(extra.get('value'))
        else:
            return toolkit.asbool(
                config.get('ckanext.forums.enabled_without_extra', True)
            )

def forum_enabled_for_organization(organization):
...
def forum_list(dataset_ref, status=issuemodel.ISSUE_STATUS.open):
    '''
    ...
    if status not in issuemodel.ISSUE_STATUS:
        log.error('forum_list status must be open or closed - got %s', status)
        status = 'open'
    ...
    issues = toolkit.get_action('forum_search')(data_dict=params)
    return issues


def forum_user_has_reported_issue(user, abuse_reports):
...
def forum_users_who_reported_issue(abuse_reports):
...
def forum_user_is_owner(user, dataset_id):
    if not user:
        # not logged in
        return False
    action = 'forum_admin'
