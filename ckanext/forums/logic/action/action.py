...
def forum_show(context, data_dict):
    ...
    p.toolkit.check_access('forum_show', context, issue_dict)
    return issue_dict
...
def forum_create(context, data_dict):
    ...
    :rtype: dictionary
    '''
    p.toolkit.check_access('forum_create', context, data_dict)
    ...
    issue.dataset_id = data_dict['dataset_id']
    session = context['session']
    issue.number = _get_next_issue_number(session, data_dict['dataset_id'])
...
@validate(schema.forum_update_schema)
def forum_update(context, data_dict):
    ...
    :rtype: dictionary
    '''
    p.toolkit.check_access('forum_update', context, data_dict)
    session = context['session']
...
def forum_delete(context, data_dict):
    ...
    '''
    p.toolkit.check_access('forum_delete', context, data_dict)
    session = context['session']
    dataset_id = data_dict['dataset_id']
    issue_number = data_dict['issue_number']
    ...
    if not issue:
        raise toolkit.ObjectNotFound(
            '{issue_number} for {dataset_id} was not found.'.format(
                issue_number=issue_number,
                dataset_id=dataset_id,
            )
        )

    issue_comment = issuemodel.IssueComment.get_comments_for_issue(issue_number)
...
def forum_search(context, data_dict):
    ...
    '''
    p.toolkit.check_access('forum_search', context, data_dict)
    user = context['user']
...
def forum_comment_create(context, data_dict):
    ...
    :rtype: dictionary
    '''
    p.toolkit.check_access('forum_comment_create', context, data_dict)
    user = context['user']
    user_obj = model.User.get(user)
...
def forum_report(context, data_dict):
    ...
    and if the number of those reaches ckanext.forums.max_strikes then issue
    ...
    :rtype: dict
    '''
    p.toolkit.check_access('forum_report', context, data_dict)
    session = context['session']
...
def _comment_or_issue_report(issue_or_comment, user_ref, dataset_id, session):
    ...
    try:
        ...
        raise ReportAlreadyExists(
            p.toolkit._('Discussion has already been reported by this user')
        )
    try:
        ...
        max_strikes = config.get('ckanext.forums.max_strikes')
        if (max_strikes
...
def forum_comment_report(context, data_dict):
    ...
    and if the number of those reaches ckanext.forums.max_strikes then comment
    will be marked as abuse and hidden.
...
@p.toolkit.side_effect_free
@validate(schema.issue_report_schema)
def forum_report_show(context, data_dict):
...
def forum_report_clear(context, data_dict):
    ...
    :type issue_number: integer
    '''
    p.toolkit.check_access('forum_report_clear', context, data_dict)
    session = context['session']
...
@validate(schema.issue_comment_report_schema)
def forum_comment_report_clear(context, data_dict):
...
def forum_comment_search(context, data_dict):
    p.toolkit.check_access('forum_comment_search', context, data_dict)
    session = context['session']
